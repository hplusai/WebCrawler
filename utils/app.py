...
from utils import SmartTypes,communication
import inspect
...
def utf_8_encoder(unicode_csv_data):
    for row in unicode_csv_data:
        yield [(isinstance(s,str) and s.encode("utf-8")) or s for s in row]
...
def __gMap(defFunc, obj, info=None):
    ...
    if isinstance(obj,(tuple,list)):
        return list(map(lambda x: __gMap(defFunc,x,info),obj))
...
#        func=lambda s : (isinstance(s,(str,unicode)) and s.decode(Dec).encode(Enc)) or s
        func=lambda s : (isinstance(s,(str,)) and s.encode(Enc)) or s
#    elif Dec:
...
LogFileName=StrChangeFileExt(AppName,'log')
...
def _Log(Msg,NewLogFileName=LogFileName,flAddTime=1, flClearLog=0, flError=0):
    try:
        ...
        Save2File(LogFName,((flAddTime and (time.strftime('%Y%m%d-%H.%M.%S',time.localtime())+' : ')) or '')+Msg+LogLineDelim,((flClearLog and 'w') or 'a'))
    except:
        pass
...
class AnyData2Csv:
    ...
    def __call__(self,data):
        Save2Csv(self.filename,data,self.fieldSeparator,self.lineSeparator)
...
class Params():
    def __init__(self,FileName=StrChangeFileExt(AppName,'ini')):
...
arrVal=lambda x : (isinstance(x,(tuple,list)) and (((len(x)==1) and arrVal(x[0])) or ((len(x)>1) and list(map(arrVal,x))))) or x or ''
#test=[1,[[1]],[],[[[1,[2,3]]]]]
#test1=arrVal(test)
...
def ApplyFunc2Vals(vals, func):
    ...
    try:
        if isinstance(vals,(str,unicode)):
            ...
            cl=vals.__class__
            return cl(list(map(lambda x : ApplyFunc2Vals(x,func),vals)))
        else:
            return func(vals)
    except:
        LogError(str(vals),1)
...
def __EncodeStrings(v,Encode='utf-8'):
    if (not isinstance(v,(str,))) and (isinstance(v,(list,tuple)) or  hasattr(v,'items')):
...
#        ret=str(Obj)
        Save2File(cacheFile,str(Obj),'wb')
        return Obj
